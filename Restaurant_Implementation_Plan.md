...
This document outlines the technical implementation plan for a responsive restaurant homepage built with Django-rendered HTML templates. The page highlights restaurant information, restaurant photos, today's recommended dishes, customer reviews, upcoming events, and basic contact/location details.

本文件概述 food/餐饮 网站首页的开发实施方案。项目采用 Django + HTML 模板实现，不做前后端分离。首页只展示餐厅介绍、餐厅照片、今日推荐菜、顾客评价、近期活动、联系方式与地址信息，不承载完整菜单。
...
* **Customer Reviews (顾客评价):** Displays owner-managed customer review highlights between recommendations and events.
    * 在今日推荐菜和活动之间展示 owner 管理的精选顾客评价。
...
* **Owner Control Dashboard (Owner 控制界面):** A protected `/owner/` dashboard for updating restaurant information, managing restaurant photos, maintaining today's recommended dishes, managing customer reviews, setting discounts, and scheduling events without writing code.
    * 提供受保护的 `/owner/` 控制界面，供所有者在不编写代码的情况下更新餐厅信息、管理餐厅照片、维护今日推荐菜、管理顾客评价、设置折扣以及安排活动。
...
### CustomerReview Model (顾客评价模型)
* `customer_name` (CharField): Customer display name.
* `quote` (TextField): Review text.
* `rating` (PositiveSmallIntegerField): Rating from 1 to 5.
* `is_visible` (BooleanField): Toggles whether the review appears on the homepage.
* `order` (IntegerField): Display order for the review section.
