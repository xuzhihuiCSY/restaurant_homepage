...
* **Optional Online Order Section (可选在线点餐区):** Displays an "Order Online" button in the hero and a dedicated section above Events when the owner enables it, provides an online order URL, and uploads two decorative images.
    * 当 owner 开启该区块、提供在线点餐链接并上传两张点缀图片时，在首页首屏显示 “Order Online” 按钮，并在 Events 上方显示专门的在线点餐区。
...
* `online_order_image_left` (ImageField): Required decorative image when `show_online_order` is True.
* `online_order_image_right` (ImageField): Required decorative image when `show_online_order` is True.
